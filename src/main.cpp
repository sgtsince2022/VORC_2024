...
void init() {
  ...
  pwm.setPWM(servo1, 0, 375); //360
  pwm.setPWM(servo2, 0, 500); //180
  pwm.setPWM(servo3, 0, 150); //180
  pwm.setPWM(servo4, 0, 375); //360
}
...
void dichuyen(){ 
  ...
  if (LY >= Move.forward_low && LY <= Move.forward_high){
    moveY = (int16_t) map(LY, Move.forward_low, Move.forward_high, 0, 2048);
    cocau.tienhoaclui = 1;
  }
  else if (LY >= Move.backward_low && LY <= Move.backward_high){
    moveY = (int16_t) map(LY, Move.backward_low, Move.backward_high, 0, -2048);
    cocau.tienhoaclui = 2;
  }
  ...
  if (RX >= Move.right_low && RX <= Move.right_high){
    moveX = (int16_t) map(RX, Move.right_low, Move.right_high, 0, 2048);
    cocau.traihoacphai = 1;
  }
  else if (RX >= Move.left_low && RX <= Move.left_high){
    moveX = (int16_t) map(RX, Move.left_low, Move.left_high, 0, -2048);
    cocau.traihoacphai = 1;
  }
  ...
  //phanh, có thể gây giật bot (không phải giật điện)
    ...
    if (cocau.tienhoaclui == 1){
      ...
      //thời gian 0,05s tránh tối đa tình trạng động cơ bị dừng khi có tín hiệu di chuyển
      delay(50); 
      ...
      cocau.tienhoaclui = 0;
      pwm.setPWM(DC1DUONG, 0, 1200);
      pwm.setPWM(DC2DUONG, 0, 1200);
      //Đảm bảo chắc chắn động cơ không quay nữa
      pwm.setPWM(DC1AM, 0, 0);
      pwm.setPWM(DC2AM, 0, 0);
      delay(50);
